(() => {
  ...
  async function sync(characterData) {
    ...
    for (const d of wanted) {
      ...
      if (!row) {
        await client.from('character_resources').insert({ character_id: characterData.id, resource_key: d.key, current_uses: d.max, max_uses: d.max, recovery: d.recovery });
      } else if (row.max_uses !== d.max || row.recovery !== d.recovery) {
        await client.from('character_resources').update({ max_uses: d.max, current_uses: Math.min(d.max, row.current_uses + Math.max(0, d.max - row.max_uses)), recovery: d.recovery }).eq('id', row.id);
      }
    }
  ...
  async function change(id, delta) {
    ...
    if (!data) return;
    const value = Math.max(0, Math.min(data.max_uses, data.current_uses + delta));
    await client.from('character_resources').update({ current_uses: value }).eq('id', id);
    await render((await character()).id);
  }
  ...
  window.restoreCharacterResources = async (characterId, restType) => {
    ...
    for (const r of data || []) {
      if (restType === 'long' || r.recovery === 'short_rest') await client.from('character_resources').update({ current_uses: r.max_uses }).eq('id', r.id);
    }
    await render(characterId);
  };

  document.addEventListener('click', event => {
    const minus = event.target.closest('.resource-minus');
    const plus = event.target.closest('.resource-plus');
    if (minus) change(minus.dataset.id, -1);
    if (plus) change(plus.dataset.id, 1);
  });

  window.addEventListener('character-rest-applied', async event => {
    const c = await character();
    if (c) await window.restoreCharacterResources(c.id, event.detail?.restType || 'long');
  });
})();
