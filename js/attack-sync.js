...
function calculateWeaponAttack(character, scores, item) {
  ...
  const notes = [
    ranged ? 'Ranged' : 'Melee',
    finesse ? `Finesse (${ability})` : '',
    properties,
    item.properties?.mastery ? `Mastery: ${item.properties.mastery}` : '',
...
async function addAttackForRow(row) {
  const characterId = document.querySelector('#equipment-modal')?.dataset.characterId;
  ...
  const [{ data: item, error: itemError }, { data: character, error: characterError }, { data: scores, error: scoresError }] = await Promise.all([
    ...
    attackSupabase.from('characters').select('id,class_key,level').eq('id', characterId).single(),
    attackSupabase.from('character_ability_scores').select('*').eq('character_id', characterId).single()
  ]);
  if (itemError || characterError || scoresError || !item) return;

  const attack = calculateWeaponAttack(character, scores, item);
  const { data: existing } = await attackSupabase.from('character_attacks').select('id').eq('character_id', characterId).eq('name', item.name).maybeSingle();
  if (existing) {
    ...
    await attackSupabase.from('character_attacks').insert({ character_id: characterId, name: item.name, attack_bonus: attack.attack_bonus, damage: attack.damage, notes: attack.notes, sort_order: Date.now() });
  }
  const message = document.querySelector('#equipment-message');
  if (message) { message.textContent = 'Attack calculated and added to the character sheet.'; message.className = 'status-message success'; }
}
...
function injectAttackButtons() {
  ...
  if (!modal || !inventory || modal.classList.contains('hidden')) return;
  ...
  rows.forEach(row => {
    ...
    button.type = 'button';
    button.className = 'icon-button inventory-attack-sync';
    button.title = 'Calculate attack';
    button.textContent = '⚔';
